 ...
 * @arg {Array<Number>|Number} [options.intents] - TODO: DOCUMENT
 */
Discord.Client = function DiscordClient(options) {
	...
	this.inviteURL = null;
	this.options = JSON.parse(JSON.stringify(options)); // deepclone the options object so we don't destroy the user's object later on
	this.connect = this.connect.bind(this, this.options);
	
	// make sure that we're only accepting arrays of intents or already computed number
	if (!this.options.intents || (!Array.isArray(this.options.intents) && !Number.isInteger(this.options.intents))) {
		throw new Error('Intents are required since v8');
	}

	if (Array.isArray(this.options.intents)) {
		this.options.intents = trasformIntentsToInteger(this.options.intents);
	}

	if (this.options.autorun === true) this.connect();
};
...
/**
 * Helper to get the guild's id from a channel id
 */
...
DCP.getAllUsers = function(callback) {
	...
	if (!this.bot) {
		if (this.options.intents && !(this.options.intents & Discord.Intents.Flags.GUILD_MEMBERS)) {
			throw new Error("Cannot request all users without the GUILD_MEMBERS intent set");
		}
		send(this._ws, Payloads.ALL_USERS(this));
	}
...
function trasformIntentsToInteger(arr) {
	return arr.reduce((acc, flag) => acc | flag, 0);
}
	...
	switch (message.t) {
		case "READY":
			...
			if (client.bot) client.getOauthInfo(function(err, res) {
				...
				client.inviteURL = "https://discord.com/oauth2/authorize?client_id=" + res.id + "&scope=bot";
			});
		...
		case "GUILD_MEMBER_UPDATE":
			if (client.servers[_data.guild_id].members) {
				old = copy(client.servers[_data.guild_id].members[_data.user.id]);
				Member.update(client, client.servers[_data.guild_id], _data);
			}
			return emit(client, message, old, client.servers[_data.guild_id].members ? client.servers[_data.guild_id].members[_data.user.id] : undefined);
		case "GUILD_MEMBER_REMOVE":
			if (_data.user && _data.user.id === client.id) return;
			client.servers[_data.guild_id].member_count -= 1;
			var member = null;
			if (client.servers[_data.guild_id].members) { 
				member = client.servers[_data.guild_id].members[_data.user.id];
			}
			emit(client, message, member);
			return member !== null ? delete(client.servers[_data.guild_id].members[_data.user.id]) : null;
...
function handleWSClose(code, data) {
	...
	client.emit("disconnect", eMsg, code, data);
}
...
function joinVoiceChannel(client, voiceSession) {
	...
	if (isNode) voiceSession.keepAliveBuffer = Buffer.alloc(8).fill(0);
...
function handlevWSMessage(voiceSession, vMessage, vFlags) {
	...
	switch (vData.op) {
		case 2: //Ready (Actually means you're READY to initiate the UDP connection)
			...
			var udpDiscPacket = Buffer.alloc(70);
...
AudioCB.VoicePacket = (function() {
	if (!isNode) return;
	var header = Buffer.alloc(12), nonce = new Uint8Array(24), output = Buffer.alloc(2048);
...
function addToStreamBuffer(RStream, data) {
	return RStream.push(Buffer.from(data)) || !!RStream.read(data.length);
}
...
Discord.Intents.Privileged = Discord.Intents.Flags.GUILD_PRESENCES | Discord.Intents.Flags.GUILD_MEMBERS;
Discord.Intents.All = Object.values(Discord.Intents.Flags).reduce((acc, flag) => acc | flag, 0);
Discord.Intents.NonPrivileged = Discord.Intents.All ^ Discord.Intents.Privileged; 
...
(function() {
	Payloads = {
		IDENTIFY: function(client) {
			return {
				op: Discord.Codes.GatewayOpCodes.IDENTIFY,
				d: {
		...
		RESUME: function(client) {
			return {
				op: Discord.Codes.GatewayOpCodes.RESUME,
				d: {
					token: client.internals.token,
		...
		HEARTBEAT: function(client) {
			return {op: Discord.Codes.GatewayOpCodes.HEARTBEAT, d: client.internals.sequence};
		},
		ALL_USERS: function(client) {
			return {op: Discord.Codes.GatewayOpCodes.ALL_USERS, d: Object.keys(client.servers)};
		},
		STATUS: function(input) {
			return {
				op: Discord.Codes.GatewayOpCodes.PRESENCE_UPDATE,
				d: {
		...
		UPDATE_VOICE: function(serverID, channelID, self_mute, self_deaf) {
			return {
				op: Discord.Codes.GatewayOpCodes.VOICE_STATE_UPDATE,
				d: {
					guild_id: serverID,
		...
		OFFLINE_USERS: function(array) {
			return {
				op: Discord.Codes.GatewayOpCodes.GUILD_PRESENCES,
				d: {
		...
		VOICE_SPEAK: function(v) {
			return {op:Discord.Codes.GatewayOpCodes.VOICE_GUILD_PING, d:{ speaking: !!v, delay: 0 }};
		},
		VOICE_IDENTIFY: function(clientID, voiceSession) {
			return {
				op: Discord.Codes.GatewayOpCodes.DISPATCH,
				d: {
		...
		VOICE_DISCOVERY: function(ip, port, mode) {
			return {
				op: Discord.Codes.GatewayOpCodes.HEARTBEAT,
				d:{
					protocol:"udp",
					data:{
						address: ip,
