...
Discord.Client = function DiscordClient(options) {
	...
	// should catch if 
	if (!options.intents || (!Array.isArray(options.intents) && !Number.isInteger(options.intents))) {
		throw new Error('Intents are required since v8');
	}

	if (Array.isArray(options.intents)) {
		options.intents = trasformIntentsToInteger(options.intents);
	}
...
Discord.Intents.Everything = Object.values(Discord.Intents.Flags).reduce((acc, flag) => acc + flag, 0);
Discord.Intents.SafeIntents = Discord.Intents.Everything ^ Discord.Intents.Privileged; 
